...
AWS_ACCOUNT=$(cat < "${tf_vars}/environment.tfvars" | grep "aws_account" | awk -F"=" '{print $2}' | awk -F'"' '{print $2}')
region="eu-west-2"
s3_backend="${client_abbr}-terraform-backend-state-${region}-${AWS_ACCOUNT}"
...
tf_validate="terraform -chdir="${tf_dir}" validate ${var_file_params} $* "
#tf_graph_plan="terraform graph $* -type=plan -draw-cycles \"${tf_dir}\" > ${base_dir}/plan.dot"
tf_plan="terraform -chdir="${tf_dir}" plan ${var_file_params} $* -input=false "
...
tf_import="terraform -chdir=\"${tf_dir}\" import ${var_file_params} '${tf_resource:-}' '${tf_resource_id:-}'"
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------
if [[ "${action}" == "move" ]]; then
  # terraform state mv command currently requires you to be in the terraform code directory
  cd "${tf_dir}"
  output_command "TF STATE:" "$tf_state_mv"
  eval $tf_state_mv
  cd "${parent_dir}"
fi